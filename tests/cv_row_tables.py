...
from prob_calibration import get_prob_calibration


ID_LENGTH = 8
DETECTION_PROB_D = 1e3 * 0.1**ID_LENGTH
...
def remove_lines_img(
    ...
    detected_lines = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, line_kernel, iterations=1)
    ...
    if cnts:
        if mode == "vertical":
            ...
            avg_x_distance = (x_right - x_left) / ID_LENGTH
    ...
    for c in cnts:
        if mode == "vertical":
            ...
            if abs((x_loc - x_left) % avg_x_distance) > 2 * (dpi // 150) and abs(
                (x_loc - x_left) % avg_x_distance - avg_x_distance
            ) > 2 * (dpi // 150):
                continue
        count_lines += 1
    ...
    if do_reconstruct:
        ...
        result = cv2.morphologyEx(
            pre_result, cv2.MORPH_CLOSE, repair_kernel, iterations=3
        )
...
#     # skip rows where SIS User ID is not an integer of length ID_LENGTH
#     # first keep the rows where SIS User ID is an integer
#     df_ids = df_ids[df_ids["SIS User ID"].apply(lambda x: str(x).isnumeric())]

#     # then keep the rows where SIS User ID is of length ID_LENGTH
#     df_ids = df_ids[df_ids["SIS User ID"].apply(lambda x: len(str(x)) == ID_LENGTH)]
...
def import_students_from_csv(filepath):
    ...
    # skip rows where student_id is not an integer of length ID_LENGTH
    # first keep the rows where student_id is an integer
    df_ids = df_ids[df_ids["student_id"].apply(lambda x: str(x).isnumeric())]

    # then keep the rows where SIS User ID is of length ID_LENGTH
    df_ids = df_ids[df_ids["student_id"].apply(lambda x: len(str(x)) == ID_LENGTH)]
    print(f"Found {len(df_ids)} students with valid University IDs.")
...
def extract_digit_boxes_from_img_new(
    ...
    sub_boundaries = get_possible_boundaries(img_rgb, padding_px)
    ...
    for boundary in sub_boundaries:
        ...
        print(
            "No contours found with the appropriate number of horizontal and vertical lines"
        )
        return None
...
def get_all_digits_new(
    ...
    for idx_submission, submission in enumerate(quizzes_img_list):
        for idx_page, page in enumerate(submission):
            digits_extracted = extract_digit_boxes_from_img_new(np.array(page), dpi=dpi)
...
def resize_and_pad_com_center(digit_img, size):
    ...
    digit_img = np.pad(
        digit_img, ((pad_above, pad_below), (pad_left, pad_right)), "constant"
    )
    return digit_img


def preprocess_digit_box(
    digit_box, threshold_uint8=20, min_distance=2, morph_element=None, morph=None
):
    ...
    digit_box_cropped = digit_box_inv[y_min:y_max, x_min:x_max]
...
def get_predicted_labels(predictions, df_ids, found_digits):
    max_probability = 0
    predicted_ufid = -1
    for ufid in df_ids["student_id"]:
        # if ufid in found_digits.keys():
        #     continue
        digit_probs = [predictions[i][int(x)] for i, x in enumerate(str(ufid))]
    ...
    #     predicted_student = df_ids[df_ids["student_id"] == predicted_ufid][
    #         "Student"
    #     ].values[0]
    # else:
    #     pass
    #     # print("No match found")
    return predicted_ufid, max_probability
...
def get_all_ufids(digit_imgs, df_ids, model, len_subs):
    ...
    # morph_elements = []
    detection_cutoff = 1e3 * 0.1**ID_LENGTH
        ...
        for morph in morph_list:
            ...
            if isinstance(model, dict):
                ...
                predictions = sess.run(
                    [output_name], {input_name: np.stack(digits_processed)}
                )[0]
                probs.append(predictions)
            else:
                raise ValueError(
                    "Model must be a dictionary with keys 'sess', 'input_name', and 'output_name'"
                )

            predicted_ufid, max_probability = get_predicted_labels(
                predictions, df_ids, found_digits
            )
            ...
            if max_probability >= detection_cutoff:
                if submission_idx in docs_classified.keys():
                    ...
                    assert docs_classified[submission_idx] == (predicted_ufid,)
                else:
                    docs_classified[submission_idx] = (predicted_ufid,)
                break
        if max_probability == 0:
            found_digits.append([submission_idx, page_idx, 0, -1, ""])
        else:
            found_digits.append(
                [
                    submission_idx,
    ...
    df_probs.to_csv("inference_probs.csv", index=False)
    df_digits = pd.DataFrame(
        found_digits,
        columns=["doc_idx", "page_idx", "max_probability", "ufid"],
    )
    get_prob_calibration("inference_probs.csv", "./data/test_ids.csv")
    df_digits = (
        df_digits.sort_values("max_probability", ascending=False)
...
def classify(
    ...
    digit_imgs = get_all_digits_new(img_list, dpi=dpi)
    print("Classifying the digits")
    df_digits = get_all_ufids(digit_imgs, df_ids, model, len_subs=len(img_list))

    return df_digits


def get_images_for_classify(
    folder_path,
    pages=None,
    crop_box=None,
):
    # for each image in the folder, use the function
    # import_crop_images to get the images of interest and crop them to the crop_box
    # return a list of images with the following format:
    # [ [image1, image2, ...], [image1, image2, ...], ... ]
    # and a list of the Document paths corresponding to each image
    images = []
    image_paths = []
    image_doc_page = []

    # import doc_info.csv from folder_path to get (doc, page, filename)
    doc_info = pd.read_csv(os.path.join(folder_path, "doc_info.csv"))
    # and group by doc and sort by page to create
    # a list of lists of image paths
    doc_info = doc_info[doc_info["page"].isin(pages)]
    doc_info = doc_info.sort_values(["doc", "page"])
    doc_info_grouped = doc_info.groupby("doc")
    for _, group in doc_info_grouped:
        image_paths.append(group["filename"].values.tolist())
        image_doc_page.append(group[["doc", "page"]].values.tolist())

    for doc_paths in image_paths:
        doc_imgs = []
        for img_path in doc_paths:
            ...
            if crop_box is not None:
                ...
                img = img[
                    crop_box_px[1] : crop_box_px[3], crop_box_px[0] : crop_box_px[2]
                ]
            doc_imgs.append(img)
        images.append(doc_imgs)

    return images, image_doc_page
...
def classify_digits(
    ...
    model_path_onnx = "./models/digits_model.onnx"
    print("Retrieving images for classification...", end="", flush=True)
    all_imgs, image_doc_page = get_images_for_classify(
        folder_path=folder_path,
        pages=pages,
        crop_box=crop_box,
    )
    print("\tDONE")
    df_ids = import_students_from_csv("./data/test_ids.csv")
    if len(df_ids) == 0:
    ...
    df_digits.to_csv("inference_results.csv", index=False)
    return df_digits


if __name__ == "__main__":
    classify_digits(
        folder_path="../imgs/q11/",
        dpi=300,
        crop_box=None,
        pages=[3],
    )
